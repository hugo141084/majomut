...
class producto extends ActiveRecord {
    ...
    public function listarXid($id) {
        return $this->find_by_sql("SELECT p.id, concat_ws(', ',p.descripcion,pre.descripcion,c.descripcion,pr.descripcion) descripcion,p.clave, p.peso, p.existencia, em.descripcion empaque, me.descripcion medida,p.precio FROM producto p inner join preparacion pr on pr.id=p.preparacion_id inner join presentacion pre on pre.id=p.preparacion_id inner join calidad c on p.calidad_id=c.id left join embalaje em on p.empaque_id=em.id left join medida me on p.medida_id=me.id  where p.id=$id");
    }

...
class detalle_pedido extends ActiveRecord {
    ...
    public function listarProductoCompra($compraId) {
         $sqlProducto = "SELECT det.id,p.id productoId, p.clave, concat_ws(', ',p.descripcion,pre.descripcion,c.descripcion,pr.descripcion) descripcion, det.cantidad, det.precio,det.impuesto,det.total,det.id linea,det.pieza,det.total,det.presentacion_venta
...
FROM inventario as inv, producto as pro  WHERE ALMACEN_ID =$id and inv.PRODUCTO_ID= pro.ID ";
    ...
    public function guardarDatos($compraId,$productoId,$cantidad,$precio,$ivaI,$totalI,$cantidadP,$presentacionVenta,$medida){
